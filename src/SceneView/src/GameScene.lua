...
function GameScene:createLayer(  )
    ...
    local str = cc.FileUtils:getInstance():getStringFromFile("path.lua")
    local xml = LuaXML_ParseFile(str);
    ...
    if scene_dir == 0 then   -- right
        ...
        mapNode:setContentSize(contentSize)
            ...
            if string.sub(j,0,5) == "Layer" then
                ...
                for k,sprite in pairs(layer) do
                    if string.sub(k,0,6) == "Sprite" then
                        local sp = cc.Sprite:create("image/"..sprite.Name.Value)
