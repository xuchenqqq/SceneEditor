...
function GameScene:createLayer(  )
     local function getConfig() 
        local f = assert(io.open("res/mapname.txt",'r')) 
        local mapname = f:read()

        f = assert(io.open("res/mappath.txt",'r'))
        local mappath = f:read()

        f = assert(io.open("res/assertpath.txt",'r'))
        local assertpath = f:read()

        f:close()
        return mapname, mappath, assertpath     
    end
    local xmlName, xmlPre, imagePre = getConfig()
    ...
    print(xmlPre.."/"..xmlName)
    local xml = LuaXML_ParseFile(xmlPre.."/"..xmlName);
    ...
    if scene_dir == 0 then   -- right
        ...
        go = cc.MoveBy:create(scene_count * scene_width/moveVel_X, cc.p(-(scene_count * scene_width-winSize.width),0))
    ...
    elseif scene_dir == 1 then  -- top
        ...
        go = cc.MoveBy:create(scene_count * scene_height/moveVel_Y, cc.p(0,-(scene_count * scene_height-winSize.height)))
    		...
    		if string.sub(j,0,5) == "Layer" then
	    		...
	    		for k,sprite in pairs(layer) do
	    			if string.sub(k,0,6) == "Sprite" then
	    				local sp = cc.Sprite:create(imagePre.."/"..sprite.Name.Value)
	    		...
	    		mapNode:addChild(view, zorder, cc.p(speed,1), cc.p(0,0))
	    	end
    	end
    end
    return mapLayer
